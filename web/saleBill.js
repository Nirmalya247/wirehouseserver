...
function getBillHtmlA4(shop, sale, items) {
    ...
    <body style="transform: scale(1); zoom: ${(process.env.PORT ? 0.72 : 1)};">
    `;
    var i = 0;
    var total = 0;
...
function getBillHtmlA4V2(shop, sale, items) {
    var header = `
        <style>
            body {
    ...
    var footer = `
        ...
        <div class="footer" style="transform: scale(1); zoom: ${(process.env.PORT ? 0.70 : 1)};">
            <div class="footer-holder">
                Page {{page}} of {{pages}} for ${shop.shopname} #${sale.id}
            </div>
        </div>
    `;
                    ...
                    <tr>
                        <td>
                            <div class="bill-to">
                                ...
                                <div class="name">${ sale.customerName } ${ sale.customerID }</div>
                                <div class="phone">${ sale.customerPhone }</div>
                            </div>
                        </td>
                        <td rowspan="2" style="vertical-align: top; width: 50%;">
                            <div class="bill">
                                ...
                                <div class="invoice-date">
                                    ...
                                    <div class="value">${ (new NepaliDate(sale.createdAt)).format('DDD MMMM DD, YYYY') }</div>
                                </div>
                                <div class="due-amount">
                                    <div class="label">Amount Due:</div>
                                    <div class="value">${ sale.creditAmount }</div>
                                </div>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <td>
                            <div class="bill-by">
                                ...
                                <div class="name">${sale.userName} ${sale.userID}</div>
                            </div>
                        </td>
                    </tr>
                </div>
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        html += `
            <tr class="item-tr item-tr-data">
                ...
                <td class="item-td item-td-amount">${items[i].totalPrice}</td>
            </tr>
        `;
    }
    var billDate = new NepaliDate(new Date(sale.createdAt));
    html += `
            </table>
            <div class="total-holder">
                <div class="total-sub-holder">
                    ...
                    <div class="total">
                        ...
                        <div class="value">${ sale.vat } %</div>
                    </div>
                    <div class="tendered">
                        <div class="label">Payment on ${ billDate.format('MMMM DD, YYYY') } using ${ sale.paymentMode }:</div>
                        <div class="value">${ sale.totalTendered } Rs.</div>
                    </div>
                    <div class="payment-due">
                        <div class="label">Amount Due:</div>
                        <div class="value">${ sale.creditAmount } Rs.</div>
                    </div>
                </div>
            </div>
        </body>
...
function getSaleBill(req, res) {
    ...
    mdb.Shop.findOne({ where: { id: 1 } }).then(resShop => {
        mdb.Sale.findOne({ where: { id: saleId } }).then(resSale => {
            mdb.SaleItem.findAll({ where: { saleId: saleId }, include: [{ model: mdb.Item, as: 'item' }] }).then(resSaleItem => {
